...
$allowedCategories = ['Meat','Seasoning','Grain','Seafood','Vegetables','Dairy','Others'];

$search_item = $_GET['item'] ?? '';
$search_category = $_GET['category'] ?? '';

$categoryStmt = $pdo->prepare("SELECT DISTINCT category FROM inventory WHERE category IN (" . implode(',', array_fill(0, count($allowedCategories), '?')) . ") ORDER BY category ASC");
$categoryStmt->execute($allowedCategories);
$categories = $categoryStmt->fetchAll(PDO::FETCH_COLUMN);

$conditions = ["i.category IN (" . implode(',', array_fill(0, count($allowedCategories), '?')) . ")"];
$params = $allowedCategories;

if ($search_item) { $conditions[] = "i.item LIKE ?"; $params[] = "%$search_item%"; }
if ($search_category) { $conditions[] = "i.category = ?"; $params[] = $search_category; }
...
$totalQuery = "SELECT COALESCE(SUM(wasted_qty),0) AS total_wasted_qty FROM inventory WHERE category IN (" . implode(',', array_fill(0, count($allowedCategories), '?')) . ")";
$totalStmt = $pdo->prepare($totalQuery);
$totalStmt->execute($allowedCategories);
