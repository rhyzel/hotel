...
$pendingPOs = $pdo->query("
    SELECT po.po_id, po.po_number, po.item, po.category, po.total_amount, po.status, 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $existingItem = $pdo->prepare("SELECT item_id, quantity_in_stock FROM inventory WHERE item = :item AND category = :category LIMIT 1");
        $existingItem->execute([':item' => $poData['item'], ':category' => $poData['category']]);
        ...
        if ($existing) {
            ...
            $insertStmt = $pdo->prepare("INSERT INTO inventory (item, category, quantity_in_stock, unit_price, inspected_by, used_qty, wasted_qty) VALUES (:item, :category, :quantity_in_stock, :unit_price, :inspected_by, 0, 0)");
            $insertStmt->execute([':item' => $poData['item'], ':category' => $poData['category'], ':quantity_in_stock' => $quantity_received, ':unit_price' => $unitPrice, ':inspected_by' => $inspected_by]);
        }
        ...
        $insertGRN = $pdo->prepare("INSERT INTO grn (po_id, po_number, item, category, quantity_received, inspected_by, condition_status, date_received, notes) VALUES (:po_id, :po_number, :item, :category, :quantity_received, :inspected_by, :condition_status, NOW(), :notes)");
        $insertGRN->execute([':po_id' => $po_id, ':po_number' => $poData['po_number'], ':item' => $poData['item'], ':category' => $poData['category'], ':quantity_received' => $quantity_received, ':inspected_by' => $inspected_by, ':condition_status' => $condition, ':notes' => $notes]);
