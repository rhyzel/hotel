...
if (!empty($_POST['item'])) {
    $item = $_POST['item'];
    $conditions[] = "item LIKE :item";
}

$low_stock_threshold = 10;

$query = "SELECT item_id, item, quantity_in_stock, unit_price
          FROM inventory
...
$reorder_items = array_map(fn($i) => $i['item'], array_slice($items, 0, 3));
if ($reorder_items) {
  ...
  <div class="container">
    ...
    <div class="search-container">
      <form method="POST" class="search-form">
        ...
        <input type="text" name="item" placeholder="Search by item" value="<?= htmlspecialchars($_POST['item'] ?? '') ?>">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr class="low-stock">
          ...
          <td data-label="Item Name"><?= htmlspecialchars($item['item']) ?></td>
