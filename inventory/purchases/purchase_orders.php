...
$latestReceived = $pdo->query("
    SELECT po.item, s.supplier_name, g.date_received 
    FROM purchase_orders po
    LEFT JOIN suppliers s ON po.supplier_id = s.supplier_id
    LEFT JOIN grn g ON po.po_number = g.po_number
    ...
    <header>
      ...
      <p>
        ...
        <?php if ($latestReceived): ?>
          <strong><?= htmlspecialchars($latestReceived['item']) ?></strong> 
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td data-label="Item Name"><?= htmlspecialchars($row['item']) ?></td>
