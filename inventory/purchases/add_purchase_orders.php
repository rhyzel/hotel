...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("INSERT INTO purchase_orders (supplier_id, po_number, item, order_date, status, total_amount, category, quantity) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$supplier_id,$po_number,$item,$order_date,$status,$total_amount,$category,$quantity]);
...
<input type="text" name="item" placeholder="Item Name" value="<?= htmlspecialchars($_POST['item'] ?? '') ?>" required>
