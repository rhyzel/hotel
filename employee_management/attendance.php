...
$holidayName = '';
$holidayCheck = $conn->prepare("SELECT name, percentage FROM holidays WHERE date = ? LIMIT 1");
$holidayCheck->bind_param("s", $date);
$holidayCheck->execute();
$holidayData = $holidayCheck->get_result()->fetch_assoc();
if ($holidayData) {
...
if (isset($_POST['punch']) && isset($_POST['local_time'])) {
    ...
    $rateStmt = $conn->prepare("SELECT daily_rate FROM staff WHERE staff_id=?");
    $rateStmt->bind_param("s", $staff_id);
    $rateStmt->execute();
    $daily_rate = $rateStmt->get_result()->fetch_assoc()['daily_rate'] ?? 0;
...
while ($row = $res->fetch_assoc()) {
    $attendance_records[$row['staff_id']] = [
        'status'=>$row['status'], 
        'time_in'=>$row['time_in'], 
...
function updateTime() {
    ...
    document.querySelectorAll('input[name="local_time"]').forEach(i => i.value = formattedTime);
}
...
<div class="container">
    ...
    <div class="holiday-info">
        <?php if ($holidayName): ?>
            ...
            <strong>Pay Multiplier:</strong> <?= $holidayPercentage ?>%
        <?php else: ?>
            No holiday today (Normal rate)
        <?php endif; ?>
    </div>
    ...
    <div style="margin:10px 0;">
        <form action="schedule.php" method="get">
            <button type="submit">View All Staff Schedule</button>
        </form>
    </div>
