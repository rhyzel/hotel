...
class ComplaintRepository {
    ...
    public function create(array $data): Complaint {
        // rating intentionally excluded for complaints
    $params = [
        ':guest_id' => $data['guest_id'] ?? null,
        ':guest_name' => $data['guest_name'] ?? '',
        ':comment' => $data['comment'] ?? '',
        ':type' => $data['type'] ?? 'complaint', // Accept any type string
    ...
    public function update(array $data): bool {
        ...
        $fields = [];
        $params = [':id' => $data['id']];
        foreach (['guest_id','guest_name','comment','type','status','reply'] as $field) {
            if (isset($data[$field])) {
                $fields[] = "$field = :$field";
