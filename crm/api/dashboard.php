...
try {
    ...
    // Loyalty members
    if ($hasLoyalty) {
        // Instead of SUM(members_count), count actual guests with a valid loyalty tier
        $stmt = $conn->query("SELECT COUNT(*) FROM guests WHERE loyalty_tier IS NOT NULL AND loyalty_tier != ''");
    ...
    if ($hasLoyalty) {
        try {
            // Show all tiers, even if 0, and use actual guest count per tier
            $stmt = $conn->query("
                SELECT lp.tier, 
                       (SELECT COUNT(*) FROM guests g WHERE g.loyalty_tier = lp.tier) AS members_count
            ...
            $dashboardData['loyalty_distribution'] = [
                ['tier' => 'bronze', 'members_count' => 0],
                ['tier' => 'silver', 'members_count' => 0],
                ['tier' => 'gold', 'members_count' => 0],
                ['tier' => 'platinum', 'members_count' => 0]
            ];
        }
    } else {
    ...
    // FRIENDLY SYSTEM: All users see the same dashboard data
