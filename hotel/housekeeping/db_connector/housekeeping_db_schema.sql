-- housekeeping_db_schema.sql
-- SQL schema for Hotel Housekeeping + Maintenance + Inventory + Staff Performance
-- Compatible with the project's PHP code (aliases: maintenance.request_id -> maintenance_id, issue_description -> issue)
-- Use MySQL / MariaDB. Run in XAMPP phpMyAdmin or `mysql < housekeeping_db_schema.sql`.

CREATE DATABASE IF NOT EXISTS `housekeepingdb` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE `housekeepingdb`;

-- disable foreign key checks during import to allow safe DROP TABLE / CREATE TABLE
...
-- Staff (housekeeping/maintenance staff)
DROP TABLE IF EXISTS `staff`;
CREATE TABLE `staff` (
  `staff_id` INT NOT NULL AUTO_INCREMENT,
  `first_name` VARCHAR(80) NOT NULL,
  `last_name` VARCHAR(80) NOT NULL,
  `role` VARCHAR(80) DEFAULT 'Housekeeping',
  `email` VARCHAR(150) DEFAULT NULL,
  `phone` VARCHAR(32) DEFAULT NULL,
  PRIMARY KEY (`staff_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Housekeeping tasks (used by TaskManager and task_assignment)
