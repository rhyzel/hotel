...
final class RoomStatusRepository
{
    ...
    public function getAll(): array
    {
        $sql = "
            ...
            LEFT JOIN rooms r ON hrs.room_id = r.room_id
            ORDER BY 
                CASE WHEN r.room_number IS NOT NULL THEN 0 ELSE 1 END,
                r.room_number ASC, hrs.room_id ASC
        ";
    ...
    public function getCounts(): array
    {
        ...
        $sql = "SELECT status, COUNT(*) AS total FROM housekeeping_room_status GROUP BY status";
        $res = $this->db->query($sql);
        if ($res) {
            while ($row = $res->fetch_assoc()) {
