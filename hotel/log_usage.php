...
$items = $pdo->query("
    SELECT item_name, SUM(quantity_in_stock) AS total_stock
    FROM inventory
    GROUP BY item_name
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("SELECT item_id, quantity_in_stock FROM inventory WHERE item_name = :item_name ORDER BY item_id ASC");
    ...
    foreach ($inventoryRows as $row) {
        ...
        $pdo->prepare("INSERT INTO stock_usage (item_id, used_qty, used_by, date_used) 
                       VALUES (:item_id, :used_qty, :used_by, NOW())")
            ->execute([
                ':item_id' => $row['item_id'],
                ':used_qty' => $deduct,
                ':used_by' => $used_by
        ...
        $pdo->prepare("UPDATE inventory 
                       SET quantity_in_stock = quantity_in_stock - :deduct, used_qty = used_qty + :deduct 
...
<div class="overlay">
  <div class="container">
    ...
    <form method="POST" class="add-form">
      ...
      <input type="text" name="used_by" placeholder="Used By" required>
