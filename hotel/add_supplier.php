...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($contact_phone)) {
        if (preg_match('/^09\d{9}$/', $contact_phone)) {
        } elseif (preg_match('/^\+639\d{9}$/', $contact_phone)) {
        } else {
            $errors[] = "Invalid Philippine phone number. Use 09xxxxxxxxx or +639xxxxxxxxx.";
        }
    }

    if (empty($errors)) {
        $stmt = $pdo->prepare("INSERT INTO suppliers (supplier_name, contact_person, email, phone, address) 
