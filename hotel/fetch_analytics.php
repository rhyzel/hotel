...
body, html { height: 100%; font-family: 'Outfit', sans-serif;
  background: url('hotelbckg.jpg') no-repeat center center fixed;
...
<div style="display:flex; gap:20px; flex-wrap:wrap;">
  <div class="card" style="height:520px; width:500px; display:flex; flex-direction:column; align-items:center; justify-content:center;">
    ...
    <button id="allRoomsBtn">All Rooms</button>
...
<tbody id="roomTableBody">
  <?php foreach($rooms as $room): ?>
    <tr>
      ...
      <td><?= htmlspecialchars($room['price_rate']) ?></td>
      <td><?= htmlspecialchars($room['created_at']) ?></td>
      <td><?= htmlspecialchars($room['updated_at']) ?></td>
      <td>
        <?php if (strtolower($room['status']) !== 'available'): ?>
          <button onclick="viewDetails(<?= $room['room_id'] ?>)" 
            style="padding:5px 10px; background:#ffc107; border:none; border-radius:6px; cursor:pointer; font-weight:600;">
            View Details
          </button>
        <?php endif; ?>
      </td>
...
const initialData = getChartData(<?= json_encode($statusCounts) ?>);
...
ctx.canvas.onclick = function(evt) {
  ...
  if (points.length) {
    ...
    currentFilter = label; // save filter
...
async function fetchDataAndUpdate() {
  try {
    ...
    // Total
    document.getElementById("totalRoomsDisplay").textContent = 
    ...
    renderTable(filtered, currentFilter === "all" ? "Room Details" : currentFilter + " Rooms");
  } catch (err) {
