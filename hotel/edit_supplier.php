...
$stmt = $pdo->prepare("SELECT supplier_id, supplier_name, contact_person, phone, email, address FROM suppliers WHERE supplier_id = :id");
$stmt->execute([':id' => $id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($phone) && !preg_match('/^(09\d{9}|\+639\d{9})$/', $phone)) {
        $errors[] = "Phone number must be in Philippine format (09XXXXXXXXX or +639XXXXXXXXX).";
    }
