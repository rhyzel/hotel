...
<div class="main-grid">
  <div class="menu-items">
    <?php foreach($lounge_items as $l):
        ...
        $quantity = isset($l['quantity_in_stock']) && is_numeric($l['quantity_in_stock']) ? (int)$l['quantity_in_stock'] : 0;
        $unavailable = $quantity <= 0;
    ?>
    <form method="post" class="menu-item <?= $unavailable ? 'unavailable' : '' ?>">
      <div class="item-image">
        <img src="<?= (!empty($l['filename']) && file_exists($server_path)) ? $url_path : 'https://via.placeholder.com/120x120?text=No+Image' ?>" alt="<?= htmlspecialchars($l['item']) ?>">
      </div>
      <div class="item-details">
        <span><?= htmlspecialchars($l['item']) ?> - ₱<?= number_format($l['unit_price'],2) ?></span>
        <?php if(!$unavailable): ?>
        ...
        <input type="hidden" name="name" value="<?= htmlspecialchars($l['item']) ?>">
